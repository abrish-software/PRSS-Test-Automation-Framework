...
class RentCancelOfficer(BaseDriver):
    ...
    def cancel_rent(self):
        click_cancel_rent = self.driver.find_element(By.XPATH, "//a[@class='btn btn-sm btn-nrlais' and text()='Cancel "
                                                               "Rent']")

        self.driver.execute_script("arguments[0].scrollIntoView(true);", click_cancel_rent)
        time.sleep(5)
        self.driver.execute_script("arguments[0].click();", click_cancel_rent)
        time.sleep(10)

    def rent_cancel_application_description(self, rcapplicationdescription):
        self.appdesc.application_description(rcapplicationdescription)
    ...
    def add_rent_cancelation_doc(self, uploadRCD, addRCDreference, addRCDdescription):
        add_RCD = self.driver.find_element(By.XPATH, '//*[@id="row_1"]/td[5]/a')
        ...
        self.driver.execute_script("arguments[0].click();", add_RCD)
        time.sleep(5)
        upload_doc.send_keys(uploadRCD)
        time.sleep(5)
        add_doc_ref.send_keys(addRCDreference)
        time.sleep(5)
        add_doc_description.send_keys(addRCDdescription)
        time.sleep(5)
    ...
    def register_cancel_rent_transaction(self, rcapplicationdescription, firstname, fathername,
                                         gfname, idfile, idtext, pomfile,
                                         pomreference, pomdescription, uploadRCD,
                                         addRCDreference, addRCDdescription, uploadLHC,
        ...
        self.cancel_rent()
        self.rent_cancel_application_description(rcapplicationdescription)
        self.next_to_second()
        self.applicant_id_info(idfile, idtext)
        self.applicant_POM_info(pomfile, pomreference, pomdescription)
        self.next_to_third()
        self.next_to_fourth()
        self.add_rent_cancelation_doc(uploadRCD, addRCDreference, addRCDdescription)
