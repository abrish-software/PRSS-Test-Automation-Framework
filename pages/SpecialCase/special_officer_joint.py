...
from pages.SpecialCase.second_holder import SecondHolder


class SpecialJointHolders(BaseDriver):
    ...
    def add_holder_info(self, firstname, fathername, gfname):
        # add_holder = self.driver.find_element(By.ID, "special_case_beneficiary_add")
        self.driver.execute_script("arguments[0].click();",
                                   self.wp.wait_until_element_to_be_clickable(By.ID, "special_case_beneficiary_add"))
        # Add holder Information
        time.sleep(5)
        add_first_name = self.driver.find_element(By.ID, "party_firstname")
        add_first_name.send_keys(firstname)
        time.sleep(3)
    ...
    def Id_Information(self, uploadid, idreference):
        ...
        self.driver.execute_script("arguments[0].click();", self.wp.wait_until_presence_of_element_located(By.XPATH,
                                                                                                           "//li["
                                                                                                           "@id"
                                                                                                           "='special_case_beneficiaries_party_picker_attachement-tab"
                                                                                                           "']/a"))
        ...
        # Add second holder info

    def add_second_holder(self, sfirstname, sfathername, sgfname, shaddress, sshareinfo, suploadid,
                          sidreference):
        second_holder = SecondHolder(self.driver)
        second_holder.register_second_holder_Information(sfirstname, sfathername, sgfname, shaddress, sshareinfo,
                                                         suploadid, sidreference)
        # click next button
    ...
    def click_on_reg_transaction(self):
        ...
        self.driver.execute_script("arguments[0].click();",
                                   self.wp.wait_until_presence_of_element_located(By.XPATH,
                                                                                  "//button[@class='btn btn-primary']"))
        time.sleep(3)

    # save special case transaction for one holder
    def register_special_case_joint_holders(self, applicationdescription, fhfirstname, fhfathername, fhgfname,
                                            fholderaddress,
                                            fhshare_info, fholderID, fhIDreference, shfirstname, shfathername, shgfname,
                                            sholderaddress, shshare_info, sholderID, shIDreference, holderclaim,
                                            holderclaimR,
                                            holderclaimD,
        ...
        self.add_holder_info(fhfirstname, fhfathername, fhgfname)
        self.add_DOB()
        self.marital_status()
        self.holder_address(fholderaddress)
        self.disability_info()
        self.family_status()
        self.share_information(fhshare_info)
        self.Id_Information(fholderID, fhIDreference)
        self.add_second_holder(shfirstname, shfathername, shgfname,
                               sholderaddress, shshare_info, sholderID, shIDreference)
        self.next_button()
        self.claim_resolution(holderclaim,
                              holderclaimR,
                              holderclaimD, )
        self.holding_proof(Holdingproof, HoldingproofR, HoldingproofD)
