...
class MortgageRepayOfficer(BaseDriver):
    ...
    def select_mortgage_repayment_transaction(self):
        ...
        time.sleep(3)
        mortgage_repayment_transaction = self.driver.find_element(By.XPATH, "//a[normalize-space()='Record Repayment']")
        self.driver.execute_script("arguments[0].click();", mortgage_repayment_transaction)
        time.sleep(5)

    def mortgage_repay_application_description(self, mrapplicationdescription):
    ...
    def add_mortgage_pay_info(self, principal, penality, interest, paymentnotes):
        principal_info = self.driver.find_element(By.ID, "txt_principal")
        principal_info.send_keys(principal)
        time.sleep(5)
        penality_info = self.driver.find_element(By.ID, "txt_penality")
        penality_info.send_keys(penality)
        time.sleep(5)
        interest_info = self.driver.find_element(By.ID, "txt_interest")
        interest_info.send_keys(interest)
        time.sleep(5)
        payment_notes = self.driver.find_element(By.ID, "txt_payment_notes")
        self.driver.execute_script("arguments[0].scrollIntoView(true);", payment_notes)
        time.sleep(2)
        payment_notes.send_keys(paymentnotes)
        time.sleep(5)
    ...
    def add_mortgage_payment_reciept(self, uploadMPR, MPRreference, MPRdescription):
        add_MPR = self.driver.find_element(By.XPATH, '//*[@id="row_1"]/td[5]/a')
        ...
        self.driver.execute_script("arguments[0].click();", add_MPR)
        time.sleep(5)
        upload_doc.send_keys(uploadMPR)
        time.sleep(5)
        add_doc_ref.send_keys(MPRreference)
        time.sleep(5)
        add_doc_description.send_keys(MPRdescription)
        time.sleep(5)
    ...
    def next_to_final(self):
        go_to_final = self.driver.find_element(By.ID, 'nexttoSixth')
    ...
    def register_mortgage_repay_transaction(self, mrapplicationdescription, firstname, fathername,
        ...
        self.select_mortgage_repayment_transaction()
        self.mortgage_repay_application_description(mrapplicationdescription)
        ...
        self.next_to_fourth()
        self.add_mortgage_pay_info(principal, penality, interest, paymentnotes)
        self.next_to_fifth()
        self.add_mortgage_payment_reciept(uploadMPR, MPRreference, MPRdescription)
        self.next_to_final()
